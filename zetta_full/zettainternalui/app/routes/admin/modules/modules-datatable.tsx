...
const columns: ColumnDef<z.infer<typeof moduleSchema>>[] = [
  {
    accessorKey: "name",
    header: "Nom",
    cell: ({ row }) => (
...

  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-wrap items-center gap-1">
        ...
        {(!row.original.lessons || row.original.lessons.length === 0) && (
          <span className="text-muted-foreground text-sm">Aucune leçon</span>
        )}
      </div>
    ),
  },
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleDelete = async () => {
        try {
          const message = await deleteModule(row.original.id);
...
function DraggableRow({ row }: { row: Row<z.infer<typeof moduleSchema>> }) {
    ...
    >
      <div className="flex items-center justify-between px-4 lg:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <AddModuleForm /> {/* Ajout du composant AddModuleForm */}
        </div>
      </div>
      <TabsContent
        value="modules"
