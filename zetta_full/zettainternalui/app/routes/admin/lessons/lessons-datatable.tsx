...
import type { lessonSchema } from "~/types/lesson";

const columns: ColumnDef<z.infer<typeof lessonSchema>>[] = [
  {
    accessorKey: "name",
    header: "Nom",
    cell: ({ row }) => (
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-1">
        {!row.original.module?.name ? (
          <span className="text-muted-foreground text-sm">Aucun Module</span>
        ) : (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleDelete = async () => {
        try {
          const message = await deleteLesson(row.original.id);
...
function DraggableRow({ row }: { row: Row<z.infer<typeof lessonSchema>> }) {
...
export function LessonsDataTable({
  data: initialData,
}: {
    ...
    >
      <div className="flex items-center justify-between px-4 lg:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <AddLessonForm /> {/* Ajout du composant AddLessonForm */}
        </div>
      </div>
      <TabsContent
        value="lessons"
