...
import { ChaptersCertificationDialog } from "./certifications-chapters-dialog";

const columns: ColumnDef<z.infer<typeof certificationSchema>>[] = [
  {
    accessorKey: "name",
    header: "Nom",
    cell: ({ row }) => (
      <div className="flex items-center gap-2">
        <div className="h-8 w-8 flex-shrink-0 rounded-lg bg-muted p-2">
          <img
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-wrap items-center gap-1">
        {row.original.formations?.map((formation) => (
          <Badge key={formation.id} variant="outline" className="px-2 py-1">
            {formation.name}
          </Badge>
        ))}
        {(!row.original.formations || row.original.formations.length === 0) && (
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-1">
        <span>
          {row.original.validity_period}{" "}
          {row.original.validity_period > 1 ? "ans" : "an"}
        </span>
      </div>
    ),
  },
  ...
  },
  {
    ...
    header: "Informations",
    cell: ({ row }) => {
      const created = new Date(row.original.created_at + " UTC");
      const now = new Date();
...

  {
    ...
    cell: ({ row }) => {
      ...
      const [openChapters, setOpenChapters] = React.useState(false);
      const { deleteCertification } = useCertificationsStore();

      const handleDelete = async () => {
        try {
          const message = await deleteCertification(row.original.id);
        ...
        <>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-32">
              ...
              <DropdownMenuItem onClick={() => setOpenChapters(true)}>
                Gérer les chapitres
              </DropdownMenuItem>
          ...
          <EditCertificationForm
            certificationId={row.original.id}
            open={openEdit}
            onOpenChange={setOpenEdit}
          />

          <ChaptersCertificationDialog
...
function DraggableRow({
  ...
  row: Row<z.infer<typeof certificationSchema>>;
}) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 lg:px-6">
        ...
        <Select defaultValue="certifications">
          <SelectTrigger
        ...
        <div className="flex items-center gap-2">
          ...
          <AddCertificationForm /> {/* Ajout du composant AddCategorieForm */}
        </div>
      </div>
      <TabsContent
