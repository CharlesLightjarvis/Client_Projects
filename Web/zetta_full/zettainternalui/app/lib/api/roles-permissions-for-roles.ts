...
// Helper function specifically for roles-permissions endpoints that return data directly
function handleRolesPermissionsResponse(response: any): any {
...
export const rolesApi = {
  ...
  async getAll(): Promise<Role[]> {
    try {
      ...
      const data = handleRolesPermissionsResponse(response);
      return data.roles || [];
    } catch (error) {
  ...
  async create(data: RoleFormData): Promise<Role> {
    try {
      ...
      const responseData = handleRolesPermissionsResponse(response);
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      handleRolesPermissionsResponse(response);
    } catch (error) {
      handleApiError(error);
    }
  },
...
export const permissionsApi = {
  ...
  async delete(id: string): Promise<void> {
    try {
      const response = await api.delete(`/api/v1/admin/permissions/${id}`);
      handleRolesPermissionsResponse(response);
    } catch (error) {
      handleApiError(error);
    }
