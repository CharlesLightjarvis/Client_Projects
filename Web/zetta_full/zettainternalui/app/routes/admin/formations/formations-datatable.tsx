...
import { AddStudentsFormationsDialog } from "./add-students-formations-dialog";
...
const columns: ColumnDef<z.infer<typeof formationSchema>>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-wrap gap-1">
        {row.original.certifications &&
        row.original.certifications.length > 0 ? (
          row.original.certifications.map((certification, index) => (
            <Badge key={index} variant="default" className="px-2 py-1">
              {certification.name}
            </Badge>
          ))
