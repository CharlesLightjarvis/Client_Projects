...
interface UpdateSessionStepperProps {
...
export function UpdateSessionStepper({
  session,
  open,
  onOpenChange,
}: UpdateSessionStepperProps) {
  ...
  // Initialize form with session data when session changes
  React.useEffect(() => {
    if (session && open) {
      console.log('📋 Initializing UpdateSessionStepper with session:', session)
      console.log('🔍 Session status value:', session.status)
      console.log('🔍 Session status type:', typeof session.status)

      // Reset basic info form
      basicInfoForm.reset({
        formation_id: session.formation.id,
        start_date: session.start_date.split('T')[0], // Extract date only
        end_date: session.end_date.split('T')[0], // Extract date only
        status: session.status,
        max_students: session.max_students,
        location: session.location,
      })

      console.log('✅ Form reset with status:', basicInfoForm.getValues('status'))

      // Set selected formation
      setSelectedFormationId(session.formation.id)

      // Reset instructors form with current instructors
      // Use current_instructors OR module_instructors
      const currentInstructors = session.current_instructors || session.module_instructors || []

      console.log('👨‍🏫 Current instructors from session:', currentInstructors)

      const moduleInstructors = currentInstructors
        .filter((mi) => mi.module?.id && mi.instructor?.id)
        .map((mi) => ({
          module_id: mi.module!.id,
          instructor_id: mi.instructor!.id,
        }))

      console.log('📝 Module instructors to set in form:', moduleInstructors)

      instructorsForm.reset({
        module_instructors: moduleInstructors,
      })
    }
  }, [session, open, basicInfoForm, instructorsForm])
  ...
  const handleSubmit = async () => {
    ...
    const result = await updateSession(session.id, dataToSend)
    ...
    if (result.success) {
      ...
      toast.error(result.message || 'Erreur lors de la mise à jour de la session')
  ...
  // Watch module_instructors changes in real-time
  const watchedInstructors = useWatch({
    control: instructorsForm.control,
    name: 'module_instructors',
  })

  // Get current instructor for a module
  const getCurrentInstructorId = (moduleId: string): string => {
    const assignments = watchedInstructors || []
    const assignment = assignments.find((a) => a.module_id === moduleId)
    return assignment?.instructor_id || ''
  }
                        ...
                        >
                          ...
                          <div className="w-64">
                            <Select
                              ...
                              value={getCurrentInstructorId(module.id!)}
                              disabled={loading}
                            >
                              <SelectTrigger>
